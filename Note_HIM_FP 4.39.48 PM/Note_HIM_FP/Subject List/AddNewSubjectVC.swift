...
class AddNewSubjectVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // set button title
        if isEditSubject {
             addSubjectBtnOL.setTitle("Save Category", for: .normal)
        }else{
            addSubjectBtnOL.setTitle("Add Category", for: .normal)
        }
    ...
    @IBAction func AddNewSubjectBtnAction(_ sender: Any) {
        ...
        if  subjectStr.trim().count > 0 {
            ...
            if isEditSubject {
                DatabaseHelper.shareInstance.editSubjectData(SubjectDict: subjectData, index: indexRow, searchText: searchStr)
                isEditSubject = false
